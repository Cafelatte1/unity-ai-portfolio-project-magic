...
public class EnemyBehavior : MonoBehaviour
{
    ...
    void ListenerChanneling(int attackIndex, bool value)
    {
        if (attackIndex > 0)
        {
            _isChanneling = value;
    ...
    class ActionDeath : ActionNode
    {
        ...
        public ActionDeath(EnemyBehavior ctx) { this.ctx = ctx; }
    ...
    {
        EnemyBehavior ctx;
        public ActionDamaged(EnemyBehavior ctx) { this.ctx = ctx; }
    ...
    {
        EnemyBehavior ctx;
        public ActionDetection(EnemyBehavior ctx) { this.ctx = ctx; }
        ...
        public override EvaluateResult Evaluate()
        {
            if (ctx._target == null)
            {
                ...
                RaycastHit2D hitArea = Physics2D.CircleCast(
                    origin,
                    0.2f,
                    direction,
    ...
    {
        EnemyBehavior ctx;
        public ActionChase(EnemyBehavior ctx) { this.ctx = ctx; }
    ...
    {
        EnemyBehavior ctx;
        public ActionAttack(EnemyBehavior ctx) { this.ctx = ctx; }

        public override EvaluateResult Evaluate()
        {
            if (ctx._target == null) return new EvaluateResult(BTNode.State.Failure, this.GetType().ToString());
            
            var attackRes = ctx.attackSystem.ExecuteAttack(0, ctx.transform, ctx.layerPlayer);
    ...
    {
        EnemyBehavior ctx;
        public ActionPatrol(EnemyBehavior ctx) { this.ctx = ctx; }
    ...
    {
        EnemyBehavior ctx;
        public ActionIDLE(EnemyBehavior ctx) { this.ctx = ctx; }
