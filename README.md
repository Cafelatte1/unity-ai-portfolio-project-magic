...
- 실제 구현 내용은 Unity용 스크립트(C#)가 담긴 Asset/Scrips/ 폴더 및 **Intel OpenVINO 기반 On-Device 추론용 스크립트(C++)**가 담긴 Asset/DLL/project-magic-localai/ 폴더 내 파일들을 확인해 주세요.
- 클로드 기반 모델들로 코드 평가를 꾸준히 진행하며 여러 부분들을 개선했었고 CodeEvaluation/ 내 히스토리를 저장해 놓았으니 참고해 주세요.
